...
const EditClients = () => {
    ...
    const handleFetch = async () => {
        try {
            const { data } = await axios.get(`http://localhost:7000/api/v1/get-single-customer/${id}`);
            const clientPreviousData = data.data; // Accessing the previous data from the response
            console.log(clientPreviousData)
            // Set the clientData state with the fetched data
            setClientData({
                customerName: clientPreviousData.customerName || '',
                email: clientPreviousData.email || '',
                contactDetails: {
                    phone: clientPreviousData.contactDetails.phone || '',
                    email: clientPreviousData.contactDetails.email || '',
                    secondaryPhone: clientPreviousData.contactDetails.secondaryPhone || '',
                    secondaryEmail: clientPreviousData.contactDetails.secondaryEmail || ''
                },
                businessInfo: {
                    businessName: clientPreviousData.businessInfo.businessName || '',
                    businessCategory: clientPreviousData.businessInfo.businessCategory || '',
                    establishmentDate: clientPreviousData.businessInfo.establishmentDate
                    ? new Date(clientPreviousData.businessInfo.establishmentDate).toISOString().split('T')[0]
                    : '',
                    head: clientPreviousData.businessInfo.head || '',
                    businessAddress: {
                        street: clientPreviousData.businessInfo.businessAddress.street || '',
                        city: clientPreviousData.businessInfo.businessAddress.city || '',
                        state: clientPreviousData.businessInfo.businessAddress.state || '',
                        postalCode: clientPreviousData.businessInfo.businessAddress.postalCode || '',
                        country: clientPreviousData.businessInfo.businessAddress.country || ''
                    }
                },
                additionalAddress: {
                    street: clientPreviousData.additionalAddress.street || '',
                    city: clientPreviousData.additionalAddress.city || '',
                    state: clientPreviousData.additionalAddress.state || '',
                    postalCode: clientPreviousData.additionalAddress.postalCode || '',
                    country: clientPreviousData.additionalAddress.country || ''
                },
                clientStatus: clientPreviousData.clientStatus || 'Active',
                followUpDate: clientPreviousData.followUpDate ? new Date(clientPreviousData.followUpDate).toISOString().split('T')[0] : '', // Format to YYYY-MM-DD
                followUp: clientPreviousData.followUp || false,
                comments: clientPreviousData.comments || [],
                Roles: clientPreviousData.Roles || []
            });
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            ...
            try {
                const { data } = await axios.put(`http://localhost:7000/api/v1/update-customer/${id}`, clientData)
